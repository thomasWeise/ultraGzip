 ...
 * waited for}). It can be used to achieve graceful termination
 * by sending the other process some commands (to stdin) to shut
 * it down. If you opened a shell, for instance, you could send
 * {@code exit} followed by a {@code newline} and then flush the
 * stream.
 *
 * @param <B>
 *          the process type
...
public interface IProcessCloser<B extends _BasicProcess> {
   ...
   *           if i/o fails
   */
  public abstract void beforeClose(final B process)
      throws IOException;
}
