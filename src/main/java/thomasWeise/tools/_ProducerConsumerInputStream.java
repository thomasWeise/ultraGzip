 ...
 * producer/consumer buffer}. If this stream is closed, it will
 * also
...
final class _ProducerConsumerInputStream extends InputStream {
   ...
   *          the buffer to read from
   */
  _ProducerConsumerInputStream(
      final ByteProducerConsumerBuffer buffer) {
    super();
  ...
  /** {@inheritDoc} */
  @Override
  public final int read(final byte[] b, final int off,
      final int len) {
  ...
  public final long skip(final long n) {
    return this.m_buffer.deleteFromBuffer(
        (int) (Math.min(Integer.MAX_VALUE, n)));
  }
